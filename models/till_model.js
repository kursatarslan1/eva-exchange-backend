...
class Till {
  ...
  static async getApartmentExpensesForAccounting(date, apartment_id) {
    const queryText = `
            WITH RECURSIVE recursive_expenses AS (
                ...
                WHERE 
                    ...
                    AND ae.expense_date <= $2::date
                    AND (
                        ae.expense_period = 0 
                        OR re.next_due_date <= $2:: date
                    )
            )
            SELECT 
            ...
            FROM 
                recursive_expenses
            WHERE 
                (DATE_TRUNC('month', next_due_date) = DATE_TRUNC('month', $2::date)
                OR next_due_date IS NULL)
                AND apartment_id = $1;
            `;
    const values = [apartment_id, date];
  ...
  static async getApartmentOneTimeExpenseForAccounting(date, apartment_id) {
    const queryText = `
  ...
  static async getDebtInfoForAccounting(date, apartment_id) {
    ...
    try {
      const result = await client.query(
        cashAndCardIncomeForCurrentMonth,
        values
      );
      const expectedResult = await client.query(
        totalExpectedQueryForCurrentMonth,
        values
      );
      return [
        result.rows[0].total_card_income,
        result.rows[0].total_cash_income,
        expectedResult.rows[0].total_expected_income,
      ];
    } catch (error) {
  ...
  static async computeAccount(date, apartment_id) {
    const apartmentExpense = await this.getApartmentExpensesForAccounting(
      date,
      apartment_id
    );
    const oneTimeApartmentExpense =
      await this.getApartmentOneTimeExpenseForAccounting(date, apartment_id);
    const debtIncome = await this.getDebtInfoForAccounting(date, apartment_id);

    let totalApartmentExpense = parseFloat(
      Number(apartmentExpense) + Number(oneTimeApartmentExpense)
    ).toFixed(2);
